...
class KasbonOtherExport
{
    protected Collection $kasbonOthers;

    public function __construct(Collection $kasbonOthers)
    {
        $this->kasbonOthers = $kasbonOthers;
    }
    ...
    public function toXlsx()
    {
        ...
        foreach ($this->kasbonOthers as $i => $kasbon) {
            $row      = $i + 2;
            $totalHpp = $kasbon->items ? $kasbon->items->sum('nilai_hpp_other_item') : 0;
            $totalCA  = $kasbon->items ? $kasbon->items->sum('nilai_kasbon') : 0;

            $sheet->setCellValue("A{$row}", $i + 1);
            $sheet->setCellValue("B{$row}", $kasbon->id_kasbon_other ?? '-');
            $sheet->setCellValue("C{$row}", $kasbon->tgl_kasbon ? $kasbon->tgl_kasbon->format('d/m/Y') : '-');
            $sheet->setCellValue("D{$row}", $kasbon->joOther ? $kasbon->joOther->no_jo_other : '-');
    ...
    public function download()
    {
        if (class_exists('\PhpOffice\PhpSpreadsheet\Spreadsheet')) {
            ...
            $filename    = 'kasbon_other_' . date('Ymd_His') . '.xlsx';
    ...
    {
        $filename = 'kasbon_other_' . date('Ymd_His') . '.csv';
        ...
        return response()->stream(function () {
            ...
            foreach ($this->kasbonOthers as $i => $kasbon) {
                fputcsv($handle, [
                    $i + 1,
                    $kasbon->id_kasbon_other ?? '-',
                    $kasbon->tgl_kasbon ? $kasbon->tgl_kasbon->format('d/m/Y') : '-',
                    $kasbon->joOther ? $kasbon->joOther->no_jo_other : '-',
                    ...
                    number_format($kasbon->items ? $kasbon->items->sum('nilai_hpp_other_item') : 0, 2, '.', ''),
