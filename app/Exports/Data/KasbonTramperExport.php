...
class KasbonTramperExport
{
    protected Collection $kasbonTrampers;

    public function __construct(Collection $kasbonTrampers)
    {
        $this->kasbonTrampers = $kasbonTrampers;
    }
    ...
    public function toXlsx()
    {
        ...
        foreach ($this->kasbonTrampers as $i => $kasbon) {
            $row      = $i + 2;
            $totalHpp = $kasbon->items ? $kasbon->items->sum('nilai_hpp_tram_item') : 0;
            $totalCA  = $kasbon->items ? $kasbon->items->sum('nilai_kasbon') : 0;

            $sheet->setCellValue("A{$row}", $i + 1);
            $sheet->setCellValue("B{$row}", $kasbon->id_kasbon_tram ?? '-');
            $sheet->setCellValue("C{$row}", $kasbon->tgl_kasbon ? $kasbon->tgl_kasbon->format('d/m/Y') : '-');
            $sheet->setCellValue("D{$row}", $kasbon->joTramper ? $kasbon->joTramper->no_jo_tram : '-');
    ...
    public function download()
    {
        if (class_exists('\PhpOffice\PhpSpreadsheet\Spreadsheet')) {
            ...
            $filename    = 'kasbon_tramper_' . date('Ymd_His') . '.xlsx';
    ...
    {
        $filename = 'kasbon_tramper_' . date('Ymd_His') . '.csv';
        ...
        return response()->stream(function () {
            ...
            foreach ($this->kasbonTrampers as $i => $kasbon) {
                fputcsv($handle, [
                    $i + 1,
                    $kasbon->id_kasbon_tram ?? '-',
                    $kasbon->tgl_kasbon ? $kasbon->tgl_kasbon->format('d/m/Y') : '-',
                    $kasbon->joTramper ? $kasbon->joTramper->no_jo_tram : '-',
                    ...
                    number_format($kasbon->items ? $kasbon->items->sum('nilai_hpp_tram_item') : 0, 2, '.', ''),
