...
class JoOtherExport
{
    protected Collection $joOthers;

    public function __construct(Collection $joOthers)
    {
        $this->joOthers = $joOthers;
    }
    ...
    public function toXlsx()
    {
        ...
        // Data rows
        foreach ($this->joOthers as $i => $jo) {
            $row       = $i + 2;
            $totalSell = $jo->items ? $jo->items->sum('hargajual_idr') : 0;

            $sheet->setCellValue("A{$row}", $i + 1);
            $sheet->setCellValue("B{$row}", $jo->tgl_jo_other ? $jo->tgl_jo_other->format('d/m/Y') : '-');
            $sheet->setCellValue("C{$row}", $jo->no_jo_other ?? '-');
            ...
            $sheet->setCellValue("G{$row}", $jo->other ? $jo->other->other : '-');
            $sheet->setCellValue("H{$row}", $jo->date_start ? $jo->date_start->format('d/m/Y') : '-');
            $sheet->setCellValue("I{$row}", $jo->date_end ? $jo->date_end->format('d/m/Y') : '-');
            ...
            foreach (['A', 'B', 'C', 'H', 'I', 'K'] as $centerCol) {
    ...
    {
        $filename = 'jo_other_' . date('Ymd_His') . '.csv';

        return response()->stream(function () {
            $handle = fopen('php://output', 'w');
            fputcsv($handle, [
                'No', 'JO Date', 'JO Number', 'Customer', 'Vessel', 'Port',
                'Other Type', 'Period Start', 'Period End', 'Title', 'Items', 'Total (IDR)',
            ]);
    ...
    public function download()
    {
        if (class_exists('\PhpOffice\PhpSpreadsheet\Spreadsheet')) {
            ...
            $filename    = 'jo_other_' . date('Ymd_His') . '.xlsx';
