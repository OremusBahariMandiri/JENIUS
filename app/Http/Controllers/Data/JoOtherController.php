...
class JoOtherController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($request->filled('no_jo')) {
                $query->where('no_jo_other', 'like', '%' . $request->no_jo . '%');
            }
            ...
            // ---- Filter: Other Type (select) ----
            if ($request->filled('id_md_other')) {
            ...
            if ($request->expectsJson()) {
                return response()->json(['success' => true, 'data' => $joOthers]);
            }
            ...
            $currentFilters = [
                ...
                'id_md_other'  => $request->get('id_md_other', ''),
            ...
            if (!empty($currentFilters['id_md_other'])) {
                $o = $others->firstWhere('id_md_other', $currentFilters['id_md_other']);
                $currentFilters['other_label'] = $o ? $o->other : $currentFilters['id_md_other'];
            }

            return view('data.jo-other.index', compact(
                'joOthers',
                'customers',
                'others',
                'ports',
            ...
            if ($request->expectsJson()) {
                return response()->json(['success' => false, 'message' => 'Error retrieving JO Others: ' . $e->getMessage()], 500);
            }
    ...
    public function export(Request $request)
    {
        ...
        $joOthers = $this->buildExportQuery($request)->get();
        ...
        if ($format === 'pdf') {
            ...
            if (class_exists('\Barryvdh\DomPDF\Facade\Pdf')) {
                $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView(
                    'data.jo-other.export_pdf',
                    compact('joOthers', 'filters')
                )
                ...
                $filename = 'jo_other_' . date('Ymd_His') . '.pdf';
                return $pdf->stream($filename);
            }

            $filename = 'jo_other_' . date('Ymd_His') . '.pdf';
            $html     = view('data.jo-other.export_pdf', compact('joOthers', 'filters'))->render();
        ...
        return (new \App\Exports\Data\JoOtherExport($joOthers))->download();
    }
    ...
    private function buildExportQuery(Request $request)
    {
        ...
        if ($request->filled('no_jo'))        $query->where('no_jo_other', 'like', '%' . $request->no_jo . '%');
        ...
        if ($request->filled('id_md_other'))  $query->where('id_md_other', $request->id_md_other);
        if ($request->filled('title'))        $query->where('title', 'like', '%' . $request->title . '%');

        return $query->orderBy('tgl_jo_other', 'desc')->orderBy('id_jo_other', 'desc');
    }
    ...
    private function activeFilters(Request $request): array
    {
        ...
        if ($request->filled('id_md_other')) {
            $o = Other::find($request->id_md_other);
            $filters['other_name'] = $o ? $o->other : $request->id_md_other;
        }
