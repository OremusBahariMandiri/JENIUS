                        ...
                        <table id="contractTable" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(!empty($currentFilters['search']))
                                            {!! str_ireplace($currentFilters['search'], '<mark>' . e($currentFilters['search']) . '</mark>', e($contract->contract)) !!}
                                        @else
                                            {{ $contract->contract }}
                                        @endif
                                    </td>
                ...
                <form id="filterForm" method="GET" action="{{ route('contract.index') }}">
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <small class="text-muted">Cari berdasarkan nomor kontrak, nama kontrak, atau nama customer.</small>
                        </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="alert alert-info">
                    <i class="fas fa-info-circle me-1"></i>
                    <small>Export akan mengekspor <strong>semua data contract</strong> (tidak ada filter aktif).</small>
                </div>
                @endif
                @php
                    $exportParams = request()->only(['search', 'customer_id', 'date_from', 'date_to', 'status']);
                    $excelUrl     = route('contract.export') . '?' . http_build_query(array_merge($exportParams, ['format' => 'excel']));
                    $pdfUrl       = route('contract.export') . '?' . http_build_query(array_merge($exportParams, ['format' => 'pdf']));
                @endphp
...
<style>
    .contractPage .card { border: none; border-radius: 10px; }
    .contractPage .card-header { border-radius: 10px 10px 0 0 !important; padding: 1rem 1.5rem; }
    .contractPage .dataTables_wrapper .dataTables_filter,
    .contractPage .dataTables_wrapper .dataTables_length { margin-bottom: 1rem !important; }
    .contractPage .dataTables_wrapper .dataTables_filter { text-align: right !important; }
    ...
    .contractPage #contractTable tbody tr:hover { background-color: #f8f9fa; cursor: pointer; }
    .contractPage .btn-sm { transition: transform 0.2s; }
    .contractPage .btn-sm:hover { transform: scale(1.1); }
    ...
    .contractPage .table-responsive { width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch; }
</style>
@endpush
