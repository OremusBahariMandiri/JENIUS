        ...
        <div class="col-md-12">
            <div class="card shadow">
                <div class="card-header text-black d-flex justify-content-between align-items-center" style="background-color: #d1fae5">
                    <span class="fw-bold"><i class="fas fa-sitemap me-2"></i>Department Data</span>
                    @if(auth()->check() && (auth()->user()->is_admin || auth()->user()->hasAccess('departemen', 'tambah')))
                        ...
                        <table id="departemenTable" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $departemen->skt_dep ?? '-' }}</td>
                                    <td>{{ $departemen->nama_dep }}</td>
                                    <td>{{ $departemen->keterangan ?? '-' }}</td>
                                    <td>
                                        <div class="d-flex gap-1 justify-content-center">
                                            @if(auth()->check() && (auth()->user()->is_admin || auth()->user()->hasAccess('departemen', 'detail')))
                                            <a href="{{ route('departemen.show', $departemen->id_md_dep) }}"
                                            ...
                                            <a href="{{ route('departemen.edit', $departemen->id_md_dep) }}"
                                               class="btn btn-sm btn-warning" data-bs-toggle="tooltip" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            @endif
                                            @if(auth()->check() && (auth()->user()->is_admin || auth()->user()->hasAccess('departemen', 'hapus')))
                                            <button type="button" class="btn btn-sm btn-danger btn-delete"
                                                    data-id="{{ $departemen->id_md_dep }}"
                                                    data-name="{{ $departemen->nama_dep }}"
                                                    data-url="{{ route('departemen.destroy', $departemen->id_md_dep) }}"
...
<style>
    ...
    .departemenPage #departemenTable tbody tr:hover { background-color: #f8f9fa; cursor: pointer; }
    .departemenPage .btn-sm { transition: transform 0.2s; }
    .departemenPage .btn-sm:hover { transform: scale(1.1); }
    .departemenPage .dataTables_wrapper .dataTables_paginate .paginate_button.current {
        background: var(--primary-green) !important; color: white !important; border: 1px solid var(--primary-green) !important;
    }
    .departemenPage .dataTables_wrapper .dataTables_paginate .paginate_button:hover {
        background: var(--primary-green) !important; color: white !important; border: 1px solid var(--primary-green) !important;
    }
</style>
@endpush
...
$(document).ready(function() {
    ...
    $(document).on('click', '.btn-delete', function(e) {
        ...
        Swal.fire({
            ...
            html: `Department <strong>${name}</strong> will be permanently deleted.`,
