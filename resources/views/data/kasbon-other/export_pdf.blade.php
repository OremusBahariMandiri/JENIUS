...
<body>
    <h2>Kasbon Other List</h2>
    <p class="subtitle">
        Printed: {{ now()->format('d/m/Y H:i') }}
        &nbsp;&mdash;&nbsp;
        Total: {{ $kasbonOthers->count() }} records
    </p>
    ...
    <table>
        ...
        <tbody>
            @forelse($kasbonOthers as $i => $kasbon)
                <tr>
                    ...
                    <td class="right">{{ number_format($kasbon->items ? $kasbon->items->sum('nilai_hpp_other_item') : 0, 2, ',', '.') }}</td>
        ...
        @if($kasbonOthers->count() > 0)
        <tfoot>
            <tr>
                <td colspan="8" style="text-align:right;">Total</td>
                <td class="center">{{ $kasbonOthers->sum(fn($k) => $k->items ? $k->items->count() : 0) }}</td>
                <td class="right">{{ number_format($kasbonOthers->sum(fn($k) => $k->items ? $k->items->sum('nilai_hpp_other_item') : 0), 2, ',', '.') }}</td>
                <td class="right">{{ number_format($kasbonOthers->sum(fn($k) => $k->items ? $k->items->sum('nilai_kasbon') : 0), 2, ',', '.') }}</td>
            </tr>
        </tfoot>
        @endif
    </table>

    <div class="footer">{{ $kasbonOthers->count() }} record(s) &mdash; {{ now()->format('d/m/Y H:i') }}</div>
</body>
</html>