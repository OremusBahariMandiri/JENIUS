...
@section('content')
                ...
                <div class="card shadow">
                    <div class="card-header text-black d-flex justify-content-between align-items-center"
                        ...
                        <div>
                            ...
                            @if (auth()->check() && (auth()->user()->is_admin || auth()->user()->hasAccess('kasbon-tramper', 'tambah')))
                                <a href="{{ route('kasbon-tramper.create') }}" class="btn btn-light">
                        ...
                        @if (!empty($currentFilters) && array_filter($currentFilters))
                            <div class="alert alert-info alert-dismissible fade show" id="filterActiveAlert">
                                ...
                                @if (!empty($currentFilters['id_jo_tram']))
                                    &nbsp;<span class="badge bg-primary">JO Tramper:
                                        {{ $currentFilters['id_jo_tram'] }}</span>
                                @endif
                                ...
                                @endif
                                <a href="{{ route('kasbon-tramper.index') }}" class="btn btn-sm btn-outline-secondary ms-2">
                                ...
                                <tbody>
                                    @forelse($kasbonTrampers as $kasbon)
                                        <tr>
                                            ...
                                            <td class="text-end">
                                                {{ number_format($kasbon->items->sum('nilai_hpp_tram_item'), 2, ',', '.') }}
                                            </td>
                                            <td class="text-end">
                                            ...
                                            <td class="text-center">
                                                <div class="d-flex gap-1 justify-content-center">
                                                    @if (auth()->check() && (auth()->user()->is_admin || auth()->user()->hasAccess('kasbon-tramper', 'detail')))
                                                        <a href="{{ route('kasbon-tramper.show', $kasbon->id) }}"
                                                            class="btn btn-sm btn-info" title="Detail">
                                                            <i class="fas fa-eye"></i>
                                                        </a>
                                                    @endif
                                                    @if (auth()->check() && (auth()->user()->is_admin || auth()->user()->hasAccess('kasbon-tramper', 'ubah')))
                                                        <a href="{{ route('kasbon-tramper.edit', $kasbon->id) }}"
                                                            class="btn btn-sm btn-warning" title="Edit">
                                                            <i class="fas fa-edit"></i>
                                                        </a>
                                                    @endif
                                                    @if (auth()->check() && (auth()->user()->is_admin || auth()->user()->hasAccess('kasbon-tramper', 'hapus')))
                                                        <button type="button" class="btn btn-sm btn-danger btn-delete"
                                                            data-id="{{ $kasbon->id }}"
                                                            data-name="{{ $kasbon->id_kasbon_tram }}"
                                                            data-url="{{ route('kasbon-tramper.destroy', $kasbon->id) }}"
                                                            title="Delete">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header text-black" style="background-color: #d1fae5">
                    <h5 class="modal-title"><i class="fas fa-filter me-2"></i>Filter Kasbon Tramper</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <form id="filterForm" method="GET" action="{{ route('kasbon-tramper.index') }}">
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">JO Tramper</label>
    ...
    <div class="modal fade" id="exportModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header text-black" style="background-color: #d1fae5">
                    <h5 class="modal-title"><i class="fas fa-download me-2"></i>Export Kasbon Tramper</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    ...
                    @else
                        <div class="alert alert-info py-2">
                            <i class="fas fa-info-circle me-1"></i>
                            <small>Export will include <strong>all Kasbon Tramper data</strong>.</small>
                        </div>
                    @endif
