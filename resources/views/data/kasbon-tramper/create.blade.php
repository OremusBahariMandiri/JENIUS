...
@section('title', 'Add Cash Advance Tramper')

@push('styles')
<style>
    .kasbonTramperCreatePage .card { border: none; border-radius: 10px; box-shadow: 0 0 20px rgba(0,0,0,0.08); }
    .kasbonTramperCreatePage .card-header { border-radius: 10px 10px 0 0 !important; padding: 1rem 1.5rem; font-weight: 600; }
    .kasbonTramperCreatePage .form-label { color: var(--text-dark); margin-bottom: 0.5rem; font-weight: 500; }
    .kasbonTramperCreatePage .btn { border-radius: 8px; font-weight: 500; transition: all 0.2s; }
    .kasbonTramperCreatePage .btn:hover { transform: translateY(-1px); box-shadow: 0 4px 8px rgba(0,0,0,0.1); }
    ...
    .add-item-form-section {
        ...
        border: 2px dashed #10b981; border-radius: 12px; padding: 25px; margin-bottom: 25px;
    }
    ...
    .add-item-form-section .form-label { font-weight: 600; color: #2c3e50; font-size: 0.875rem; }
    .currency-group { display: flex; align-items: center; gap: 5px; }
    .currency-label {
        background-color: #2c3e50; color: white; padding: 0.6rem 0.75rem;
        font-size: 0.7rem; border-radius: 6px 0 0 6px;
    ...
    .table-items thead th {
        ...
        padding: 12px 10px; font-weight: 600; font-size: 0.875rem;
    ...
    .currency-label-footer {
        background-color: #2c3e50; color: white; border: 1px solid rgba(255,255,255,0.3);
        padding: 0.6rem 0.75rem; font-size: 0.85rem; border-radius: 6px;
        min-width: 55px; text-align: center; font-weight: 600;
    }
    ...
    .btn-add-to-table {
        ...
        padding: 12px 30px; border-radius: 25px; font-weight: 600; font-size: 1rem;
        box-shadow: 0 4px 15px rgba(16,185,129,0.4); transition: all 0.3s;
    }
    .btn-add-to-table:hover { transform: translateY(-3px); box-shadow: 0 6px 20px rgba(16,185,129,0.5); color: white; }
    .final-save-section {
        position: sticky; bottom: 0; background: white; padding: 20px;
        box-shadow: 0 -4px 20px rgba(0,0,0,0.1); border-radius: 12px 12px 0 0; margin-top: 30px; z-index: 100;
    }
    .btn-final-back {
        background: linear-gradient(135deg,#868686,#5e5e5e); border: none; color: white;
        padding: 15px 40px; border-radius: 12px; font-weight: 700; font-size: 1.1rem;
        box-shadow: 0 6px 20px rgba(27,27,27,0.4); transition: all 0.3s;
    }
    ...
    .items-card-disabled::after {
        ...
        background: rgba(0,0,0,0.8); color: white; padding: 20px 30px; border-radius: 10px;
        font-size: 1.1rem; font-weight: 600; z-index: 1000; box-shadow: 0 4px 20px rgba(0,0,0,0.3);
    }
    .jo-summary-card {
        background: #fff; border: 1.5px solid #10b981; border-radius: 12px;
        padding: 1.25rem 1.5rem; animation: fadeInDown 0.35s ease-out;
    }
...
<div class="container-fluid kasbonTramperCreatePage">
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="card shadow mb-4">
                <div class="card-header text-black d-flex justify-content-between align-items-center"
                    ...
                    <span class="fw-bold">
                        <i class="fas fa-plus-circle me-2"></i>Add Cash Advance Tramper
                    </span>
            ...
            <input type="hidden" id="current_kasbon_tramper_id" value="">
            <input type="hidden" id="current_kasbon_tram_str" value="">
                ...
                <div class="card shadow mb-4 items-card-disabled" id="itemsCard">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="add-item-form-section">
                            ...
                            <div class="row g-3">
                                <div class="col-md-12">
                                    <label class="form-label required-field">JO Tramper Item</label>
                                    <select id="input_jo_tram_item" class="form-select">
                                        <option value="">-- Save header &amp; select JO Tramper first --</option>
                                    </select>
        ...
        <div class="modal-content" style="border-radius:12px; border:none;">
            <div class="modal-header" style="background:#d1fae5; border-radius:12px 12px 0 0; border-bottom:none; padding:1.25rem 1.5rem;">
                <div>
                    <h5 class="modal-title fw-bold mb-0" style="color:#2c3e50;">
                        <i class="fas fa-list-ul me-2" style="color:#059669;"></i>JO Tramper Items
                    </h5>
            ...
            <div class="modal-body" style="padding:1.5rem; min-height:220px;">
                ...
                <div id="joItemsModalEmpty" style="display:none; text-align:center; padding:40px 20px; color:#6c757d;">
                    ...
                    <p class="fw-bold mb-0">No items found for this JO Tramper</p>
                </div>
            </div>
...
<script>
let currentKasbonTramperId  = null;
let currentKasbonTramStr    = '';
...
function formatRupiah(value) {
    ...
    return parts.length > 1 ? integerPart + ',' + decimalPart : integerPart + ',00';
}
...
$(document).ready(function () {
    ...
    $('#id_jo_tram').on('change', function () {
        ...
        if (!id) { joTramperItems = []; $('#input_jo_tram_item').html('<option value="">-- Select JO Tramper first --</option>'); $('#joSummaryWrapper').hide(); return; }
        ...
        loadJoTramperItems(id);
    });

    $('#input_jo_tram_item').on('change', function () {
        const itemId = $(this).val();
        if (!itemId) { $('#input_nilai_hpp').val(''); $('#hppHint').html(''); return; }
        const item = joTramperItems.find(i => i.id_jo_tram_item == itemId);
        if (item) {
    ...
    $(document).on('click', '#btnJoDetail', function () {
        const id = $('#id_jo_tram').val(); if (!id) return;
        ...
        if (joTramperItems.length > 0) { renderJoItemsModal(joTramperItems); }
        else {
            $.ajax({ url: '{{ route("kasbon-tramper.jo-items.get") }}', method: 'GET', data: { id_jo_tram: id },
...
function loadJoTramperItems(idJoTram) {
    $.ajax({
        url: '{{ route("kasbon-tramper.jo-items.get") }}', method: 'GET', data: { id_jo_tram: idJoTram },
        success: function (r) {
            if (r.success) {
                joTramperItems = r.data;
                let opts = '<option value="">-- Select Item --</option>';
                r.data.forEach(item => { opts += `<option value="${item.id_jo_tram_item}">${item.invoice_typ} (${item.invoice_ctg})</option>`; });
                $('#input_jo_tram_item').html(opts);
                const n = r.data.length;
                $('#joSummaryItemCount').text(n + ' item' + (n !== 1 ? 's' : ''));
            } else { $('#joSummaryItemCount').text('—'); }
        },
        error: function () { showFloatingAlert('error', 'Failed to load JO Tramper items'); $('#joSummaryItemCount').text('—'); }
    });
}
...
$('#btnSaveHeader').on('click', function () {
    const idJoTram = $('#id_jo_tram').val(), idDep = $('#id_md_dep').val(),
          idCabang = $('#id_md_cabang').val(), idRelease = $('#id_md_release').val(),
          tglKasbon = $('#tgl_kasbon').val();
    if (!idJoTram || !idDep || !idCabang || !idRelease || !tglKasbon) { showFloatingAlert('error', 'Please fill in all required fields'); return; }
    showFloatingAlert('saving', 'Saving header...');
    $('#btnSaveHeader').prop('disabled', true);
    $.ajax({
        url: '{{ route("kasbon-tramper.header.store") }}', method: 'POST',
        data: { id_jo_tram: idJoTram, id_md_dep: idDep, id_md_cabang: idCabang, id_md_release: idRelease,
...
$('#btnAddToTable').on('click', function () {
    ...
    const itemId = $('#input_jo_tram_item').val(), nilaiHpp = parseRupiah($('#input_nilai_hpp').val()),
          nilaiKasbon = parseRupiah($('#input_nilai_kasbon').val());
    if (!itemId) { showFloatingAlert('error', 'Please select a JO Tramper Item'); return; }
    ...
    $.ajax({
        url: '{{ route("kasbon-tramper.item.store") }}', method: 'POST',
        data: { id_kasbon_tram: currentKasbonTramStr, id_jo_tram_item: itemId, nilai_kasbon: nilaiKasbon, _token: $('input[name="_token"]').val() },
        success: function (r) {
...
function appendItemRow(d) {
    ...
    const joItem = joTramperItems.find(i => i.id_jo_tram_item == d.id_jo_tram_item) || {};
    $('#itemsTableBody').append(`
    <tr class="item-row" data-item-id="${d.id_kasbon_tram_item}">
        <td class="item-number-cell">${globalItemNumber}</td>
        <td class="text-start">${joItem.invoice_typ || d.id_jo_tram_item}</td>
        <td>${joItem.invoice_ctg || '-'}</td>
        <td>${formatNumber(d.nilai_hpp_tram_item)}</td>
        <td>${formatNumber(d.nilai_kasbon)}</td>
        <td>${formatNumber(d.total_kasbon)}</td>
        <td class="text-center">
            <button type="button" class="btn btn-danger btn-sm btn-remove-row"
                onclick="removeItem(this, '${d.id_kasbon_tram_item}')">
...
function clearItemForm() {
    $('#input_jo_tram_item').val('').trigger('change');
...
function removeItem(button, itemId) {
    ...
    $.ajax({
        url: `/kasbon-tramper/item/destroy/${itemId}`, method: 'DELETE',
        data: { _token: $('meta[name="csrf-token"]').attr('content') },
        success: function (r) {
            if (r.success) {
                showFloatingAlert('success', 'Item deleted!');
                $(button).closest('tr').remove(); renumberAllItems(); updateGrandTotal();
...
$('#resetAllBtn').on('click', function () {
    ...
    Promise.all(ids.map(id => $.ajax({ url: `/kasbon-tramper/item/destroy/${id}`, method: 'DELETE', data: { _token: $('meta[name="csrf-token"]').attr('content') } })))
    .then(() => {
...
function renumberAllItems() {
    $('.item-row').each(function (i) { $(this).find('.item-number-cell').text(i + 1); });
...
function updateGrandTotal() {
    ...
    $('.item-row').each(function () {
        ...
        totalHPP += parseRupiah(tds.eq(3).text()); totalKasbon += parseRupiah(tds.eq(4).text()); totalRemaining += parseRupiah(tds.eq(5).text());
    });
