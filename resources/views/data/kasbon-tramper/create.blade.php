...
@push('styles')
    <style>
        .kasbonTramperCreatePage .card {
            border: none;
            border-radius: 10px;
            box-shadow: 0 0 20px rgba(0, 0, 0, 0.08);
        }

        .kasbonTramperCreatePage .card-header {
            border-radius: 10px 10px 0 0 !important;
            padding: 1rem 1.5rem;
            font-weight: 600;
        }

        .kasbonTramperCreatePage .form-label {
            color: var(--text-dark);
            margin-bottom: 0.5rem;
            font-weight: 500;
        }

        .kasbonTramperCreatePage .btn {
            border-radius: 8px;
            font-weight: 500;
            transition: all 0.2s;
        }

        .kasbonTramperCreatePage .btn:hover {
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#id_jo_tram').on('change', function() {
                ...
                if (!id) {
                    joTramperItems = [];
                    $('#input_jo_tram_item').html(
                        '<option value="">-- Select JO Tramper first --</option>');
            ...
            $(document).on('click', '#btnJoDetail', function() {
                ...
                if (joTramperItems.length > 0) {
                    renderJoItemsModal(joTramperItems);
                } else {
                    $.ajax({
            ...
            $.ajax({
                ...
                success: function(r) {
                    if (r.success) {
                        ...
                        r.data.forEach(item => {
                            opts +=
                                `<option value="${item.id_jo_tram_item}">${item.invoice_typ} (${item.invoice_ctg})</option>`;
                        });
                ...
                error: function() {
                    showFloatingAlert('error', 'Failed to load JO Tramper items');
                    $('#joSummaryItemCount').text('—');
                }
            });
        }
        ...
        $('#btnSaveHeader').on('click', function() {
            const idJoTram = $('#id_jo_tram').val(); // ← id_jo_tram, bukan id_jo_cont
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        // Set state setelah header berhasil disimpan
                        currentKasbonTramperId = response.data.id;
                        currentKasbonTramStr = response.data.id_kasbon_tram;
                        isHeaderSaved = true;
        ...
        $('#btnAddToTable').on('click', function() {
            ...
            const itemId = $('#input_jo_tram_item').val(),
                nilaiHpp = parseRupiah($('#input_nilai_hpp').val()),
                nilaiKasbon = parseRupiah($('#input_nilai_kasbon').val());
            if (!itemId) {
                showFloatingAlert('error', 'Please select a JO Tramper Item');
                return;
            }
            ...
            $.ajax({
                url: '{{ route('kasbon-tramper.item.store') }}',
                method: 'POST',
                data: {
                    id_kasbon_tram: currentKasbonTramStr,
                    id_jo_tram_item: itemId,
    ...
    <tr class="item-row" data-item-id="${d.id_kasbon_tram_item}">
        ...
        function removeItem(button, itemId) {
            ...
            $.ajax({
                url: `/kasbon-tramper/item/destroy/${itemId}`,
                method: 'DELETE',
                data: {
        ...
        $('#resetAllBtn').on('click', function() {
            ...
            Promise.all(ids.map(id => $.ajax({
                    url: `/kasbon-tramper/item/destroy/${id}`,
                    method: 'DELETE',
                    data: {
                        _token: $('meta[name="csrf-token"]').attr('content')
                    }
