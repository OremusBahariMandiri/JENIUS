    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('jo-contract.index') }}" class="row g-3">
                <div class="col-md-5">
                    <input type="text" name="search" class="form-control" placeholder="Search JO contract..." value="{{ request('search') }}">
                </div>
                <div class="col-md-3">
                    <select name="id_md_cont" class="form-select">
                        <option value="">All Contracts</option>
                        @foreach($contracts as $contract)
                        <option value="{{ $contract->id_md_cont }}" {{ request('id_md_cont') == $contract->id_md_cont ? 'selected' : '' }}>
                            {{ $contract->no_contract }} - {{ $contract->customer ? $contract->customer->customer : '' }}
                        </option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-2">
                    <select name="id_md_area" class="form-select">
                        ...
                        <option value="{{ $area->id_md_area }}" {{ request('id_md_area') == $area->id_md_area ? 'selected' : '' }}>
                            {{ $area->area }}
                        </option>
                        @endforeach
                    </select>
        ...
        <div class="card-body">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @if($joContract->note)
                                <br><small class="text-muted">{{ Str::limit($joContract->note, 50) }}</small>
                                @endif
                            </td>
                            <td>
                            ...
                            <td>
                                @if($joContract->area)
                                <span class="badge bg-info text-dark">{{ $joContract->area->area }}</span>
                                @else
                                -
                                @endif
                            </td>
                            <td class="text-center">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('jo-contract.show', $joContract->id_jo_cont) }}" class="btn btn-sm btn-info" title="Detail">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('jo-contract.edit', $joContract->id_jo_cont) }}" class="btn btn-sm btn-warning" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <button type="button" class="btn btn-sm btn-danger" onclick="deleteJoContract({{ $joContract->id_jo_cont }})" title="Delete">
            ...
            <div class="d-flex justify-content-between align-items-center mt-3">
                <div class="text-muted">
                    Showing {{ $joContracts->firstItem() ?? 0 }} to {{ $joContracts->lastItem() ?? 0 }} of {{ $joContracts->total() }} entries
                </div>
                {{ $joContracts->links() }}
            </div>
        </div>
