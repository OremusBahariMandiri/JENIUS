...
<body>
    ...
    <h2>JO Tramper List</h2>
    <p class="subtitle">
        Printed: {{ now()->format('d/m/Y H:i') }}
        &nbsp;&mdash;&nbsp;
        Total: {{ $joTrampers->count() }} records
    </p>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="11%" class="center">Total (IDR)</th>
            </tr>
        </thead>
        <tbody>
            @forelse($joTrampers as $i => $jo)
                @php
                ...
                <tr>
                    ...
                    <td class="center">{{ $jo->tgl_jo_tram ? $jo->tgl_jo_tram->format('d/m/y') : '-' }}</td>
                    <td>{{ $jo->no_jo_tram ?? '-' }}</td>
        ...
        @if ($joTrampers->count() > 0)
            <tfoot>
                <tr>
                    <td colspan="8" class="right" style="text-align:right">Total</td>
                    <td class="center">{{ $joTrampers->sum(fn($jo) => $jo->items ? $jo->items->count() : 0) }}</td>
                    <td class="right">
                        {{ number_format($joTrampers->sum(fn($jo) => $jo->items ? $jo->items->sum('hargajual_idr') : 0), 2, ',', '.') }}
                    </td>
                </tr>
            </tfoot>
        @endif
    ...
    <div class="footer">
        {{ $joTrampers->count() }} record(s) &mdash; {{ now()->format('d/m/Y H:i') }}
    </div>
