...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .kop-table td, .kop-table th { border:1px solid #000; padding:5px 10px; vertical-align:middle; font-size:8.5pt; }
        .logo-cell { width:30%; text-align:center; vertical-align:middle; padding:10px 12px; border:1px solid #000; }
        .logo-cell img { max-width:110px; max-height:55px; display:block; margin:0 auto 4px; }
        .logo-cell .company-name { font-weight:bold; font-size:7.5pt; line-height:1.3; margin-top:3px; }
        .lpj-title { font-size:13pt; font-weight:bold; text-align:center; padding:10px 6px; border:1px solid #000; }
        .items-table { width:100%; border-collapse:collapse; margin-top:6px; }
        .items-table th { background-color:#2c3e50; color:#fff; border:1px solid #000; padding:6px 8px; font-size:8pt; text-align:center; font-weight:bold; }
        .items-table td { border:1px solid #000; padding:5px 8px; font-size:8pt; vertical-align:middle; }
        .items-table .cat-row td { background-color:#f0f0f0; font-weight:bold; font-size:8pt; color:#2c3e50; }
        .items-table .text-right  { text-align:right; white-space:nowrap; }
        .items-table .text-center { text-align:center; }
        .items-table .blank-row td { height:16px; }
        .total-row td { font-weight:bold; border:1px solid #000; padding:6px 8px; font-size:8.5pt; }
        .ttd-table { width:100%; border-collapse:collapse; margin-top:8px; }
        .ttd-table td { border:1px solid #000; vertical-align:top; padding:5px 8px; font-size:8pt; }
...
@php
    ...
    foreach ($lpj->kasbons as $lpjKasbon) {
        ...
        foreach ($kasbon->items as $kasbonItem) {
            ...
            $invoiceTyp = optional(optional($kasbonItem->joTramperItem)->invoice)->invoice_typ ?? $kasbonItem->id_kasbon_tram_item;
        ...
        <td>
            @foreach($lpj->kasbons as $lpjKasbon)
                @if($lpjKasbon->kasbonTramper)
                    {{ $lpjKasbon->kasbonTramper->id_kasbon_tram }}@if(!$loop->last), @endif
                @endif
            @endforeach
        </td>
    </tr>
...
<tbody>
    @foreach($kasbonOrder as $kNo)
        @php $catOrder = $kasbonGroups[$kNo]['catOrder']; $catGroups = $kasbonGroups[$kNo]['catGroups']; @endphp
