        ...
        <div class="col-lg-12">
            ...
            <div class="card shadow mb-4">
                <div class="card-header text-black d-flex justify-content-between align-items-center"
                    ...
                    <span class="fw-bold">
                        <i class="fas fa-plus-circle me-2" style="color:#059669;"></i>Add LPJ Tramper
                    </span>
...
function loadKasbonsByJo(idJoTram) {
    ...
    $.ajax({
        url: '{{ route("lpj-tramper.kasbons-by-jo") }}',
        method: 'GET',
                    ...
                    <tr class="kasbon-row" id="krow_${idx}">
                        <td class="text-center">
                            <input type="checkbox" class="form-check-input kasbon-check"
                                name="kasbon_ids[]" value="${k.id_kasbon_tram}" checked
                                data-idx="${idx}" data-amount="${k.total_kasbon}"
                                onchange="onKasbonCheck(this)">
                        </td>
                        <td class="fw-semibold" style="color:#2c3e50;">${k.id_kasbon_tram}</td>
                        ...
                        <td class="text-center">
                            <button type="button" class="btn btn-sm" title="View CA Detail"
                                ...
                                onclick="showKasbonDetail('${k.id_kasbon_tram}', ${idx})">
...
function showKasbonDetail(kasbonId, idx) {
    ...
    $.ajax({
        url: '{{ route("lpj-tramper.kasbons-by-jo") }}',
        method: 'GET',
        data: { id_jo_tram: $('#id_jo_tram').val(), detail_kasbon: kasbonId },
        success: function(r) {
            if (r.success && r.kasbon_detail) {
                ...
                items.forEach((item, i) => {
                    totalHpp    += parseFloat(item.nilai_hpp_tram_item) || 0;
                    ...
                    html += `<tr class="detail-item-row">
                        ...
                        <td class="text-end">IDR ${formatNumber(item.nilai_hpp_tram_item)}</td>
...
function populateJoSummary(idJoTram) {
    ...
    const $opt = $('#id_jo_tram option:selected');
...
$('#btnSaveHeader').on('click', function() {
    ...
    $.ajax({
        url: '{{ route("lpj-tramper.header.store") }}',
        method: 'POST',
        data: formData,
        contentType: false,
        processData: false,
...
$(document).ready(function() {
    ...
    $('#id_jo_tram').on('change', function() { populateJoSummary($(this).val()); });
});
</script>
@endpush