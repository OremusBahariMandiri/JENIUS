...
@php
    $mergedItemsJs = collect($mergedItems)->map(
        fn($i) => [
            ...
            'id_lpj_tram_item' => $i['id_lpj_tram_item'],
...
@push('styles')
    <style>
        /* === identik dengan lpj-contract/edit, hanya class lpjEditPage === */
        .lpjEditPage .card {
            border: none;
            border-radius: 10px;
...
@section('content')
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p class="text-muted small mb-3">
                        The following cash advances for JO <strong>{{ $lpj->joTramper->no_jo_tram ?? '' }}</strong>
                    ...
                    <div class="table-responsive">
                        ...
                        <div class="lpj-origin-note-tram mt-2" id="lpjOriginNoteTram" style="display:none;">
            ...
            <div class="col-lg-12">
                ...
                <div class="card shadow mb-4">
                    <div class="card-header text-black d-flex justify-content-between align-items-center"
                        ...
                        <span class="fw-bold">
                            <i class="fas fa-edit me-2" style="color:#059669;"></i>Edit LPJ Tramper
                        </span>
                        <a href="{{ route('lpj-tramper.index') }}" class="btn btn-light btn-sm">
                ...
                <input type="hidden" id="currentLpjTramStr" value="{{ $lpj->id_lpj_tram }}">
                <input type="hidden" id="currentJoTramId" value="{{ $lpj->id_jo_tram }}">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control fw-bold" value="{{ $lpj->no_lpj_tram }}"
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" class="form-control" readonly style="background:#e9ecef;"
                                    value="{{ $lpj->joTramper->no_jo_tram ?? '' }} — {{ $lpj->joTramper->title ?? '' }}">
                                        ...
                                        @foreach ($lpj->kasbons as $lpjKasbon)
                                            @if ($lpjKasbon->kasbonTramper)
                                                <span class="badge"
                                                    ...
                                                    <i
                                                        class="fas fa-check me-1"></i>{{ $lpjKasbon->kasbonTramper->id_kasbon_tram }}
                                                </span>
                                            @endif
                                        @endforeach
                                    </div>
                ...
                <div class="card shadow mb-4" id="itemsCard">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="input-item-card" id="inputItemCard">
                            ...
                            <input type="hidden" id="activeKasbonTramItemId" value="">
    ...
    <div class="final-save-section">
        <div class="d-flex justify-content-between align-items-center">
            <div></div>
            <div class="d-flex gap-2">
                <a href="{{ route('lpj-tramper.export-pdf', $lpj->id) }}" target="_blank" class="btn"
                ...
                <a href="{{ route('lpj-tramper.index') }}" class="btn btn-final-back">
                    <i class="fas fa-arrow-left me-1"></i> Back
                </a>
            </div>
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="p-3 rounded-3 mb-4 d-flex align-items-center gap-3 flex-wrap"
                        ...
                        @endif
                        <a href="{{ route('jo-tramper.edit', $lpj->joTramper->id_jo_tram ?? 0) }}" target="_blank"
                    ...
                    <div class="row g-2">
                        <div class="col-md-12">
                            ...
                            <select id="newItemKasbonTram" class="form-select select2-coa">
                                <option value="">-- Select Cash Advance --</option>
                                @foreach ($lpj->kasbons as $lpjKasbon)
                                    @if ($lpjKasbon->kasbonTramper)
                                        <option value="{{ $lpjKasbon->kasbonTramper->id_kasbon_tram }}">
                                            {{ $lpjKasbon->kasbonTramper->id_kasbon_tram }}
                                        </option>
                                    @endif
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            <div class="d-flex align-items-center gap-2 mb-3">
                                <div
                                    ...
                                    <i class="fas fa-ship" style="color:#16a34a;font-size:.8rem;"></i>
                                </div>
                                <span
...
@push('scripts')
    ...
    <script>
        ...
        const currentLpjTramStr = '{{ $lpj->id_lpj_tram }}';
        const csrfToken = $('meta[name="csrf-token"]').attr('content');
        const bulkSaveUrl = '{{ route('lpj-tramper.items.bulk-save', $lpj->id) }}';
        const storeNewItemUrl = '{{ route('lpj-tramper.item.store-new', $lpj->id) }}';
        const updateHeaderUrl = '/lpj-tramper/header/update/{{ $lpj->id }}';
        const refreshKasbonsUrl = '{{ route('lpj-tramper.refresh-kasbons', $lpj->id) }}';
        const addKasbonsUrl = '{{ route('lpj-tramper.add-kasbons', $lpj->id) }}';
        ...
        function renderTable() {
            ...
            mergedItems.forEach(item => {
                const kNo = item.id_kasbon_tram_no || '—';
            ...
            kasbonOrder.forEach(kNo => {
                ...
                catOrder.forEach(category => {
                    ...
                    items.forEach(item => {
                        ...
                        const isOrigin = !!item.origin_lpj_tram; // ← CEK origin
                    ...
                    <i class="fas fa-trash"></i></button>`;
                        ...
                        // ← TAMBAH class row-from-lpj jika isOrigin
                    ...
                    <tr class="${rowClass}" id="row_${gIdx}" data-index="${gIdx}">
                        ...
                        <td class="cell-readonly cell-right">${formatNumber(item.nilai_hpp_tram_item)}</td>
            ...
            // ← TOGGLE note biru
            $('#lpjOriginNoteTram').toggle(hasOriginItems);
            updateFooter();
        }
        ...
        function updateFooter() {
            ...
            mergedItems.forEach(i => {
                totalHpp += parseFloat(i.nilai_hpp_tram_item) || 0;
        ...
        function openEditItem(index) {
            ...
            $('#infoKasbonNo').text(item.id_kasbon_tram_no);
            ...
            $('#activeHpp').val(item.nilai_hpp_tram_item);
        ...
        function saveItemInput() {
            const kasbonTramItemId = $('#activeKasbonTramItemId').val();
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    items: [{
                        id_kasbon_tram_item: kasbonTramItemId,
        ...
        $('#btnSaveHeader').on('click', function() {
            ...
            formData.append('id_jo_tram', $('#currentJoTramId').val());
            ...
            $.ajax({
                ...
                success: function(r) {
                    ...
                    r.new_kasbons.forEach(k => {
                        $tbody.append(`<tr>
                        <td class="text-center"><input type="checkbox" class="kasbon-check" value="${k.id_kasbon_tram}" checked></td>
                        <td class="fw-semibold">${k.id_kasbon_tram}</td>
        ...
        $('#btnAddNewItem').on('click', function() {
            const kasbonTram = $('#newItemKasbonTram').val();
            ...
            const errors = [];
            if (!kasbonTram || !cat || !invoiceId) errors.push('Cash Advance, Category, dan Item wajib diisi.');
            ...
            $.ajax({
                ...
                success: function(r) {
                    ...
                    if (r.success) {
                        ...
                        renderTable();
                        $('#newItemKasbonTram, #newItemCategory, #newItemCoa').val('');
